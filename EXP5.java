...
public class EXP5 {
	...
	public void Election()
	{
		...
		while (true) {
			if (process[newer].act) {
				System.out.println("Process " + process[old].id + " pass Coordinator(" + coord + ") message to process " + process[newer].id);
				old = newer;
			}
			newer = (newer + 1) % TotalProcess;
			if (newer == coord) {
	...
	{
		EXP5 object = new EXP5();
		object.initialiseEXP5();
		object.Election();
	}
}
